...
<c:forEach begin="0" end ="${acteursModel.acteurs.size()/6}" var="i">	
	<div class="row">
		<c:forEach begin="0" end ="5" var="j">
			<div class="col-sm-2 col-md-4S">
				<c:set var="k" scope="session" value="${(i*6)+j}"/>		
				<c:url value="/acteur" var="url">
					<c:param name="idActeur" value="${acteursModel.acteurs[k].id}"/>
				</c:url>
				<c:if test="${k<acteursModel.acteurs.size()}">							
					<c:if test="${not empty acteursModel.acteurs[k].photo}">
						<div class="thumbnail">
							<a href="${url}"><img src ="https://image.tmdb.org/t/p/w500${acteursModel.acteurs[k].photo}" class="img-thumbnail img-responsive"/></a>
						</div>
					</c:if>											 
					<div class="caption">  						
			    		<h4><a href="${url}" class="titreClickable">${acteursModel.acteurs[k].nom}</a></h4>	
			    		<p><a href="${url}" class="titreClickable">${acteursModel.acteurs[k].lieuNaissance}</a></p>		    		
		    		</div>
	    		</c:if>
	    	</div>	    
	    </c:forEach>
